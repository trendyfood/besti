  ...
  foreach( $orders as $order ):
    ...
    if(  0 > $remains ):
      ...
      if( $statu == "canceled" || $statu == "cancel" ):
        ...
        $report = $report->execute(array("date"=>date("Y-m-d H:i:s"),"id"=>$order["client_id"],"action"=>"#".$orderid." O número do pedido foi cancelado e ".$order["order_charge"]." Valor reembolsado. Saldo antigo:".$clientBalance." / novo saldo:".$user["balance"],"ip"=>"127.0.0.1" ));
      ...
      elseif( $statu == 'complete' || $statu == 'completed' ):
        ...
        $report = $conn->prepare("INSERT INTO client_report SET client_id=:id, action=:action, report_ip=:ip, report_date=:date ");
        $report = $report->execute(array("date"=>date("Y-m-d H:i:s"),"id"=>$order["client_id"],"action"=>"#".$orderid." O número do pedido foi concluído.","ip"=>"127.0.0.1" ));
      ...
      elseif( $statu == "partial" ):
        ...
        $report = $conn->prepare("INSERT INTO client_report SET client_id=:id, action=:action, report_ip=:ip, report_date=:date ");
        $report = $report->execute(array("date"=>date("Y-m-d H:i:s"),"id"=>$order["client_id"],"action"=>"#".$orderid." O pedido numerado é marcado como parcial e ".$return_price." Valor reembolsado. Saldo antigo:".$clientBalance." / novo saldo:".$user["balance"],"ip"=>"127.0.0.1" ));
